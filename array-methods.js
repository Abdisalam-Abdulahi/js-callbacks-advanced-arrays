...
runners.forEach((name) => {
  const completeName = name.first_name + " " + name.last_name;
...
let allCaps = [];
for (let initialName of runners) {
  const convertingNames = initialName.first_name;
  allCaps.push(convertingNames);
}
const lastRESULT = allCaps.map((x) => x.toUpperCase());
...
const result = runners.filter((runner) => runner.shirt_size == "L");
...
const amountOfDonation = [];
runners.forEach((amount) => {
  amountOfDonation.push(amount.donation);
});

const ticketPriceTotal = amountOfDonation.reduce(
...
// Problem 1
// i wanna list first name of all players in alphapetical order
const orderd = [];
runners.forEach((runner) => {
  let firstNameOfRunners = runner.first_name;
  orderd.push(firstNameOfRunners);
});
console.log(orderd.sort());
// Problem 2
// I printed to the console to all of the campany name and company email
const nameAndEmails = [];
runners.forEach((runner) => {
  const nAndE = `${runner.company_name} ${runner.email}`;
  nameAndEmails.push(nAndE);
});
console.log(nameAndEmails);
// Problem 3
//I printed how many campanied donated less than 100
smallDonaters = [];
runners.forEach((runner) => {
  const amountDonated = runner.donation;
  if (amountDonated < 100) {
    smallDonaters.push(runner.company_name);
  }
});
console.log(`${smallDonaters.length} comapnies donated less 100 dollars`)
